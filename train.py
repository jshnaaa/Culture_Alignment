...
def main():
    ...
    # 获取专家利用率统计
    logger.info("Computing expert utilization statistics...")
    expert_stats = model.get_expert_utilization_stats(test_dataloader)
    logger.info(f"Expert utilization: {expert_stats['utilization']}")

    # 绘制专家利用率
    plt.figure(figsize=(10, 6))
    expert_ids = range(args.num_experts)
    utilization = expert_stats['utilization'].cpu().numpy()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.output_dir, "expert_utilization.png"))
    plt.close()
